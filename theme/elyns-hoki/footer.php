<?php
/**
 * Site Footer
 * @package elyns-hoki
 */
$whatsapp     = elyns_get( 'elyns_phone', '6287845195050' );
$phone_disp   = elyns_get( 'elyns_phone_display', '0878 4519 5050' );
$email        = elyns_get( 'elyns_email', 'sales@elynshoki.com' );
$address      = elyns_get( 'elyns_address', 'GRIYA AIRA TANDEAN BLOK E 11, JL. K F TANDEAN BANDAR UTAMA, TEBING TINGGI, SUMUT 20613' );
$footer_desc  = elyns_get( 'elyns_footer_desc', 'PT. ELYNS HOLONG KOMODITI supplies selected Indonesian agricultural commodities for business and export needs.' );
$copyright    = elyns_get( 'elyns_footer_copyright', 'Copyright &copy; ' . date('Y') . ' PT. ELYNS HOLONG KOMODITI. All rights reserved.' );
?>

<footer class="site-footer" role="contentinfo">
    <div class="container">
        <div class="footer-grid">

            <!-- Brand Col -->
            <div class="footer-col footer-col__brand">
                <a href="<?php echo esc_url( home_url('/') ); ?>" class="site-logo" rel="home">
                    <?php if ( has_custom_logo() ) : ?>
                        <?php the_custom_logo(); ?>
                    <?php else : ?>
                        <div class="site-logo__icon" aria-hidden="true">
                            <svg viewBox="0 0 24 24" fill="white" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 2C7.4 2 3 5.7 3 10.5c0 3.6 2.2 6.7 5.4 8.1L12 22l3.6-3.4C18.8 17.2 21 14.1 21 10.5 21 5.7 16.6 2 12 2zm0 2c3.3 0 7 2.8 7 6.5 0 2.9-1.8 5.4-4.5 6.6L12 19l-2.5-1.9C6.8 15.9 5 13.4 5 10.5 5 6.8 8.7 4 12 4z"/>
                            </svg>
                        </div>
                        <div class="site-logo__text">
                            <span class="site-logo__name">PT. ELYNS HOLONG</span>
                            <span class="site-logo__tagline">Agricultural Commodities</span>
                        </div>
                    <?php endif; ?>
                </a>
                <p class="footer-desc"><?php echo wp_kses_post( $footer_desc ); ?></p>

                <a href="https://wa.me/<?php echo esc_attr( preg_replace('/\D/', '', $whatsapp) ); ?>"
                   class="btn btn--ghost btn--sm"
                   target="_blank" rel="noopener noreferrer">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413z"/></svg>
                    Chat on WhatsApp
                </a>
            </div>

            <!-- Quick Links -->
            <div class="footer-col">
                <p class="footer-col__title">Quick Links</p>
                <nav class="footer-links" aria-label="Footer navigation">
                    <?php
                    wp_nav_menu([
                        'theme_location' => 'primary',
                        'container'      => false,
                        'menu_class'     => '',
                        'depth'          => 1,
                        'fallback_cb'    => function() {
                            $pages = [
                                '/'                        => 'Home',
                                '/about'                   => 'About Us',
                                '/products'                => 'Products',
                                '/quality-sustainability'  => 'Quality & Sustainability',
                                '/gallery'                 => 'Gallery',
                                '/contact'                 => 'Contact',
                            ];
                            foreach ( $pages as $slug => $name ) {
                                echo '<a href="' . esc_url(home_url($slug)) . '">' . esc_html($name) . '</a>';
                            }
                        },
                    ]);
                    ?>
                </nav>
            </div>

            <!-- Products -->
            <div class="footer-col">
                <p class="footer-col__title">Our Products</p>
                <div class="footer-links">
                    <?php
                    $products_query = elyns_get_products( 10 );
                    if ( $products_query->have_posts() ) :
                        while ( $products_query->have_posts() ) : $products_query->the_post();
                            echo '<a href="' . esc_url( get_permalink() ) . '">' . esc_html( get_the_title() ) . '</a>';
                        endwhile;
                        wp_reset_postdata();
                    else :
                        echo '<a href="' . esc_url( home_url( '/products' ) ) . '">' . esc_html__( 'View Products', 'elyns-hoki' ) . '</a>';
                    endif;
                    ?>
                </div>
            </div>

            <!-- Contact -->
            <div class="footer-col">
                <p class="footer-col__title">Contact Us</p>
                <div class="footer-contact">
                    <div class="footer-contact-item">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                        <span><?php echo esc_html( $address ); ?></span>
                    </div>
                    <div class="footer-contact-item">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.1h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9a16 16 0 0 0 6.29 6.29l.82-.82a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
                        <a href="https://wa.me/<?php echo esc_attr( preg_replace('/\D/', '', $whatsapp) ); ?>" target="_blank" rel="noopener">
                            <?php echo esc_html( $phone_disp ); ?> (WhatsApp)
                        </a>
                    </div>
                    <div class="footer-contact-item">
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                        <a href="mailto:<?php echo esc_attr( $email ); ?>"><?php echo esc_html( $email ); ?></a>
                    </div>
                </div>
            </div>

        </div><!-- /.footer-grid -->

        <div class="footer-bottom">
            <span><?php echo wp_kses_post( $copyright ); ?></span>
            <span>Indonesian Agricultural Commodity Exporter</span>
        </div>
    </div>
</footer>

<!-- WhatsApp Floating Button -->
<a href="https://wa.me/<?php echo esc_attr( preg_replace('/\D/', '', $whatsapp) ); ?>?text=<?php echo urlencode('Hello, I would like to inquire about your agricultural products.'); ?>"
   class="whatsapp-float"
   target="_blank"
   rel="noopener noreferrer"
   aria-label="Chat on WhatsApp">
    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
    </svg>
</a>

<?php wp_footer(); ?>
</body>
</html>
